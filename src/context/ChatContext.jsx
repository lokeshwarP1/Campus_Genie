...
export const ChatProvider = ({ children }) => {
  ...
  const fetchChatHistory = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:5000/api/chat/history/${user?._id}`);
  ...
  const clearChatHistory = async () => {
    try {
      setIsLoading(true);
      await axios.delete(`http://127.0.0.1:5000/api/chat/history/${user?._id}`);
      setMessages([]);
    } catch (error) {
  ...
  const rateMessage = async (messageId, rating) => {
    try {
      ...
      await axios.post('http://127.0.0.1:5000/api/rate', { 
        messageId, 
        rating,
        userId: user?._id 
      });
