...
async def login_to_kmit_netra(mobile_number):
    async with async_playwright() as p:
        ...
        try:
            # 1. Go to login page
            print("[INFO] Opening KMIT Netra login page...")
            await page.goto("http://kmit-netra.teleuniv.in/", wait_until="load")
            ...
            try:
                ...
                print("[ERROR] Error: Could not find login inputs.")
            ...
            print("\n[INFO] Navigating to attendance page...")
            await page.goto("http://kmit-netra.teleuniv.in/student/attendance", wait_until="networkidle")
            ...
            try:
                ...
                print("[INFO] Waiting for attendance page header...")
                await page.wait_for_selector('.ant-page-header-heading-title', timeout=3000)
            except Exception as e:
                print("[WARN] Could not find attendance page header:", e)
            ...
            print("\n[INFO] Navigating to timetable page...")
            await page.goto("http://kmit-netra.teleuniv.in/student/time-table", wait_until="networkidle")
            await page.wait_for_selector('.ant-page-header-heading-title', timeout=3000)
...
async def extract_attendance(page):
    print("\n[INFO] Extracting attendance details...")
    ...
    try:
        print("[INFO] Expanding 'Overall' section...")
        overall_xpath = '//div[contains(@class, "ant-collapse-header") and .//h4[text()="Overall"]]'
        await page.wait_for_selector(overall_xpath, timeout=3000)
        await page.click(overall_xpath)
        ...
        print("[WARN] Error expanding 'Overall' section:", e)

    # 2. Extract Overall Attendance Percentage
    try:
        print("[INFO] Extracting Overall Attendance Percentage...")
        ...
        if overall_percentage is not None:
            print(f"\n[STATS] Overall Attendance Percentage: {overall_percentage}%")
        else:
            print("[WARN] Could not extract Overall Attendance Percentage.")
    except Exception as e:
        print("[WARN] Could not extract Overall Attendance Percentage:", e)

    # 3. Extract Attendance for Each Period on the Last Working Day
    try:
        print("[INFO] Extracting Today's Attendance...")
        ...
        print("\n[INFO] Attendance for Each Period on the Last Working Day:")
        for idx, status in enumerate(today_attendance, start=1):
            print(f"Session {idx}: {status}")
    except Exception as e:
        print("[WARN] Could not extract Today's Attendance:", e)

    return data


async def extract_timetable(page):
    print("\n[INFO] Extracting timetable details...")
    ...
    try:
        ...
        print("[INFO] Expanding all collapsible sections...")
        collapsible_headers = await page.query_selector_all('//div[contains(@class, "ant-collapse-header")]')
        for header in collapsible_headers:
            await header.click()
        ...
        print("[WARN] Could not extract timetable data:", e)
